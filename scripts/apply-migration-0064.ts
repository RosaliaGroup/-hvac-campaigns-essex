/**
 * apply-migration-0064.ts — hand-apply migration 0064 (Opportunity Center P1: 6
 ...
 * committed drizzle/0064_opportunity_center_p1.sql so the applied DDL is exactly
 ...
 *
 * Usage:
 *   railway run --service=<svc> --environment=production npx tsx scripts/apply-migration-0064.ts
 *   railway run --service=<svc> --environment=production npx tsx scripts/apply-migration-0064.ts --execute --yes-write
 */
...
const sqlPath = join(here, "..", "drizzle", "0064_opportunity_center_p1.sql");
...
async function main() {
  ...
  try {
    ...
    console.log("\n=== DDL to apply (from drizzle/0064_opportunity_center_p1.sql) ===");
