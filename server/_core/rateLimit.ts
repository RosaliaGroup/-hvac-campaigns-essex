 ...
 *
 * Same trusted-hop-from-the-right approach as `getClientIp` above (that one
 * landed on main independently, from a separate fix; this one is what
 * teamAuth.ts's login rate limiting actually calls, and additionally accepts
 * an explicit `hops` override) — kept both rather than merge them into one
 * signature while rebasing this PR, to avoid changing behavior for either's
 * existing callers.
 *
