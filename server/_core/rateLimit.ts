 ...
 *
 * REBASE NOTE (this function supersedes the trusted-hop-from-the-RIGHT
 * approach this same PR branch's own earlier commit used, and that
 * `getClientIp` below still uses — see the flag raised in the rebase PR
 * description / commit message: `getClientIp`, already merged to main
 * separately, was verified here to pick the wrong entry on Railway and risks
 * bucketing every real user into one shared rate-limit key).
 */
